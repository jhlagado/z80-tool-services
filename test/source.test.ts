...
import {
  ...
  Z80_SOURCE_PART_ORDINAL_BYTE_MAX,
  Z80_SOURCE_PART_ORDINAL_BYTE_MIN,
  Z80_ZERO_BASED_SOURCE_PART_CAPACITY,
  Z80_ZERO_BASED_SOURCE_PART_ORDINAL_MAX,
  Z80_ZERO_BASED_SOURCE_PART_ORDINAL_MIN,
} from '../src/index.js';

describe('source byte provider primitives', () => {
  it('exports the resident byte-domain source-part capacity', () => {
    expect(Z80_SOURCE_PART_ORDINAL_BYTE_MIN).toBe(0);
    expect(Z80_SOURCE_PART_ORDINAL_BYTE_MAX).toBe(0xff);
    expect(Z80_ZERO_BASED_SOURCE_PART_ORDINAL_MIN).toBe(0);
    expect(Z80_ZERO_BASED_SOURCE_PART_ORDINAL_MAX).toBe(0xfe);
    expect(Z80_ZERO_BASED_SOURCE_PART_CAPACITY).toBe(255);
    expect(Z80_ONE_BASED_SOURCE_PART_ORDINAL_MIN).toBe(1);
    expect(Z80_ONE_BASED_SOURCE_PART_ORDINAL_MAX).toBe(0xff);
    expect(Z80_ONE_BASED_SOURCE_PART_CAPACITY).toBe(255);
