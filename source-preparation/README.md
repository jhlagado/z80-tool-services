...
reader, deterministic resolver, path-keyed placement join, capacity checks, and
immutable provenance records. Language behavior enters through a profile
object. Atom's `%` grammar and masking implementation remain in Atom's host
profile. Nucleus's leading comment-shaped import convention remains in the
Nucleus profile.
...
The shared contract and operational limits are documented in
[`docs/specifications/z80-source-preparation.md`](../../../docs/specifications/z80-source-preparation.md).
Atom's [`host-source-preparation.md`](../../atom/docs/host-source-preparation.md)
documents Atom's profile-specific masking, `%INCLUDE`, `%DEFINE`,
conditional-source, and `INCBIN` behavior.
